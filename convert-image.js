...
w480.forEach((fileName) => {
  Jimp.read(`${directory}/${fileName}`, (err, file) => {
    if (err) throw err;
    file
      .resize(480, Jimp.AUTO) // resize with auto height
      .quality(60) // set JPEG quality
      .write(`${directory}/w480-${fileName}`); // save
  });
});
